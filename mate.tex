Una \emph{variable aleatoria} es una función real $X: \Omega\mapsto\mathbb{R}$ tal que el conjunto $\{\omega\in\Omega:X(\omega)\in I\}$ es un evento de $\Omega$ para cada $I\subset\mathbb{R}$, en un espacio $\Omega$ hipotético. Se consideran variables aleatorias discretas cuando su rango de valores $R_x$ es finito o contablemente infinito, las variables aleatorias continuas pueden tomar cualquier valor real en un intervalo.
...
%\end{equation}
La forma más natural de expresar la distribución de variables aleatorias discretas es la \emph{función de probabilidad}\cite{blitz19}*, para una $X$ discreta, es la función $p_X$ dada por $p_X(x)=P(X=x)$. 
