...
\bibliography{pro}
\usepackage{tikz} %PARA EL TRIÁNGULO, QUITAR
\begin{document}
\maketitle
\begin{abstract}
  \begin{center}%PARA EL TRIÁNGULO, QUITAR
Ignoren el formato, también el triángulo.\\
    ...
    \end{tikzpicture}
\end{center}%PARA EL TRIÁNGULO, QUITAR
\end{abstract}
\pagebreak
\tableofcontents
\section {incompleto}\ \\
...
\end{equation}
Primera esperanza de éxito \emph{FS}, podemos definir a $Y\sim FS(p)$ como $Y=X+1$ donde $X\sim Geom(p)$, por lo que tenemos
\begin{equation}
E(Y)=E(X+1)=\frac{q}{p}+1=\frac{1}{p}.
\end{equation}

Las \emph{distribuciones binominales negativas} generalizan la distribución geométrica en lugar de esperar por un éxito, podemos esperar por cualquier número predeterminado $r$ de éxitos.
438 p160
